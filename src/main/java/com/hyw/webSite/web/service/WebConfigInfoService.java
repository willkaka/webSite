...
//            eventInfo.setSelectedValue(CollectionUtil.getMapFirstOrNull(webElementDto.getDataMap()));
    ...
    public List<WebElementDto> getWebConfigElement(RequestDto requestDto, String elementParent, String elementArea) {
        ...
        for(WebElement webElement : webElementList){
            ...
            if(StringUtil.isNotBlank(dataString)) {
                ...
                setDefaultValue(webElementDto, jsonData, curInputValue);
                setDataValue(webElementDto, jsonData, curInputValue);
            }
            //配置为空，则将传入的返回。
            ...
            setEventInfo(webElementDto,webElement,requestDto,changedEleMap, inputValue);
     ...
     * @param inputValue
     */
    private void setEventInfo(WebElementDto webElementDto,WebElement webElement,RequestDto requestDto,
                              Map<String,Object> changedEleMap,Map<String,String> inputValue){
        List<EventInfo> eventInfos = new ArrayList<>();
        webElementDto.setEventInfoList(eventInfos);

        String configEvent = webElement.getEvent();
        if(StringUtil.isBlank(configEvent)) return;
    ...
    private void setDefaultValue(WebElementDto webElementDto,JSONObject jsonData,Map<String,String> curInputValue) {
        //默认值
    ...
    private void setDataValue(WebElementDto webElementDto,JSONObject jsonData,Map<String,String> curInputValue){
        ...
        if("optionList".equals(dataType)) {
            ...
            if(StringUtil.isNotBlank(dataBase)){
                if("#".equals(dataBase.substring(0,1))){
                    ...
                    String dataBaseName = curInputValue.get(dataBaseNameKey);
                    String libNameKey = libName.substring(1);;
                    String libNameN = curInputValue.get(libNameKey);
