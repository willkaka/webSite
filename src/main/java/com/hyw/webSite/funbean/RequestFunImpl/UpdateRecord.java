...
public class UpdateRecord implements RequestFun {
    ...
    public ReturnDto execute(RequestDto requestDto){
        ...
        List<String> keyFields = DbUtil.getTablePrimaryKeys(connection,null,tableName);
        ...
        for(String fieldName:inputValue.keySet()){
            if("modal".equals( fieldName.substring(0,5)) ){
                ...
                sql.append(fieldName.substring(5,fieldName.length())).append("=").append("\"").append(inputValue.get(fieldName)).append("\" ");
            }
        }
